...
Four annotation types in `.assay` files:
...
1. **Your `.assay` files** — `external effects` declarations you write for your project
...
This is also the right mechanism for **FFI functions**. If your project has Erlang or JavaScript FFI implementations, the checker can't analyze the foreign code and will flag those calls as `[Unknown]`. Declare their effects with `external effects` in your `.assay` file:
...
Since externals are loaded before inference, `assay infer` will propagate the declared effects through callers correctly — no `[Unknown]` noise.
...
assay ships with versioned catalog files for common Gleam packages, so you get effect knowledge out of the box without writing `external effects` declarations for standard libraries.
...
For packages not in the catalog, use `external effects` declarations in your project's `.assay` files:
...
To add a new package to the bundled catalog, create `priv/catalog/{package}@{version}.assay` with `external effects` declarations for its modules and functions. Only one version file per package is needed — the version resolution algorithm handles older installations.
